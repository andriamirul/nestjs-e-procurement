...
export class InfrastructureModule implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    useContainer(this.app.select(VendorModule), {
    ...
    console.log(
      `building microservice in  ${service.vendor.host}:${service.vendor.port}`,
    );
    this.app.connectMicroservice<MicroserviceOptions>(
      {
        transport: Transport.TCP,
        options: {
          host: service.vendor.host,
          port: Number(service.vendor.port),
        },
      },
      {
